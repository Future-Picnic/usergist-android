...
import studio.ritmus.feedback.internal.model.FrequencyCaps
...
internal class FrequencyCapStore(
        ...
        synchronized(lock) {
            ...
            if (perPrompt != null && perPrompt > 0) {
                ...
                if (state[promptId].orEmpty().any { it >= cutoff }) {
                    RitmusLogger.d("FrequencyCap: blocked $promptId by per-prompt cap ($perPrompt d)")
                    return false
                }
            }
            ...
            if (perUser != null && perUser > 0) {
                ...
                if (state[USER_CAP_KEY].orEmpty().any { it >= cutoff }) {
                    RitmusLogger.d("FrequencyCap: blocked $promptId by per-user cap ($perUser d)")
                    return false
                }
            }
            return true
    ...
    private fun persistLocked(state: Map<String, MutableList<Long>>) {
        ...
        val text = try {
            ...
            RitmusLogger.w("FrequencyCapStore.persist encode failed", e)
            return
        }
