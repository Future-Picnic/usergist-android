...
internal class PromptPresenter(
    ...
    fun present(prompt: ClientPrompt): Boolean {
        ...
        if (activity == null) {
            RitmusLogger.d("PromptPresenter: no resumed FragmentActivity; skipping prompt ${prompt.id}")
            return false
        }
        val fm = activity.supportFragmentManager
        if (fm.isStateSaved || fm.isDestroyed) {
            RitmusLogger.d("PromptPresenter: fragment manager not ready; skipping prompt ${prompt.id}")
            return false
        }
        return try {
            ...
            RitmusLogger.w("PromptPresenter.present failed", e)
            false
        }
    }

    companion object {
        private const val PROMPT_FRAGMENT_TAG: String = "studio.ritmus.feedback.prompt"
    }
}
