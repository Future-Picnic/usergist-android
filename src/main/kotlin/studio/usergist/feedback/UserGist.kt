...
import studio.usergist.feedback.internal.Identity
...
object UserGist {
    ...
    fun identify(userId: String, properties: Map<String, Any?>? = null) {
        ...
        UserGistLogger.d("UserGist.identify: externalId=${updated.externalId}")
    ...
    fun setConsent(purposes: Consent) {
        ...
        store.set(purposes)
        UserGistLogger.d("UserGist.setConsent: $purposes")
    ...
    fun getAvailableSurveys(
        ...
        UserGistLogger.d("getAvailableSurveys: survey renderer not yet available on Android")
        callback(emptyList())
    }
    ...
    fun handleSurveyDeepLink(uri: android.net.Uri): Boolean {
        ...
        UserGistLogger.d("survey.deep-link token=${token.take(8)}…")
        return true
    }
    ...
    private fun doInitialize(
        ...
        if (app != null) {
            ...
            val presenter = PromptPresenter(lifecycle).apply {
                onPromptShown = this@UserGist.onPromptShown
                onResponse = this@UserGist.onResponse
            }
    ...
    private suspend fun flushOnce() {
        ...
        if (!consent.allowsTransport()) {
            UserGistLogger.d("UserGist.flush skipped — consent.feedback not granted")
            return
        }
        ...
        while (queue.size() > 0) {
            ...
            if (success) {
                batch.consume()
                UserGistLogger.d("UserGist.flush sent ${batch.events.size} events")
            } else {
                UserGistLogger.d("UserGist.flush failed for ${batch.events.size} events; will retry")
                break
            }
        }
    }
    ...
    private suspend fun refreshArmedTriggers() {
        val consent = consentRef.get() ?: return
        if (!consent.allowsTransport()) {
            UserGistLogger.d("UserGist.refreshArmedTriggers skipped — consent.feedback not granted")
            return
        }
    ...
    private fun enqueueIdentifyServer(identity: Identity, properties: JsonObject?) {
        ...
        scope.launch {
            try {
                ...
                UserGistLogger.w("UserGist.identify send failed", e)
            }
        }
    }
